...
const getTrips = async (userId, tripId) => {
  ...
  const dbTrips = (await knex.raw(knexComplexQuery(select, table, joins, conditions), { userId, tripId }))[0];
...
const getPings = async (tripIds) => {
  ...
  try {
    ...
    const dbPings = (await knex.raw(knexComplexQuery(select, table, joins, conditions), [ ...tripIds ]))[0];

...
exports.register = async (req, res) => {
  try {
    ...
    const [ userId ] = await knex(knexTables.users).insert({ password: cryptedPwd, email, permission: PERMISSION_LEVEL.ADMIN });
    await knex(knexTables.userProfiles).insert({ userId, fullName, birthDay });

    const user = { userId, password: cryptedPwd, email, permission: PERMISSION_LEVEL.ADMIN, fullName, birthDay };
    return res.status(200).send(jwtfy(user));
  } catch(err) {

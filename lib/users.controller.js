...
exports.authenticate = async (req, res) => {
  try {
    const { userId = 0, token, email, password } = req.query;
...
exports.register = async (req, res) => {
  try {
    ...
    const [ userId ] = await knex(knexTables.users).insert({ password: cryptedPwd, email });
    await knex(knexTables.userProfiles).insert({ userId, fullName, birthDay });

    const user = { userId, email, fullName, birthDay, permission: 1 };
    return res.status(200).send(jwtfy(user));
  } catch(err) {
